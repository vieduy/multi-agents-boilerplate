...
def invoke_tool(tool_name: str, arguments: dict[str, Any]) -> Any:
    ...
    for api_path in api_paths:
        try:
            logger.info(f"📡 Attempting to call: {api_path}")
            logger.info(f"📦 Request payload: {json.dumps(event, indent=2)}")
            ...
            with urllib.request.urlopen(req, timeout=60) as resp:
                ...
                logger.info(f"✅ Tool executor response: {body}")
            ...
            last_error = e
            logger.warning(f"⚠️  HTTP error {e.code} calling {api_path}: {e}")
            if e.code == 404 and api_path == api_paths[0]:
                logger.info(f"🔄 Trying legacy endpoint...")
                continue
            try:
            ...
            last_error = e
            logger.error(f"❌ OSError calling {api_path}: {e}")
